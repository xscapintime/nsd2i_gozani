...
for p in paths:
    mats = glob.glob(f'../{p}.d*.enhancer.5k.100b.gz')
    ...
    for m in mats:
        ...
        df_center = pd.concat([df_combined.iloc[:, 45:55], df_combined.iloc[:,100:]], axis=1).groupby([100,101]).mean()
    ...
    #     palette=["#87CEFA", "#CDC9C9"],  saturation=0.7, linewidth=1)
    ...
    # nobs = ["mean=" + str(f"{i:.3f}") for i in df_mean.to_list()]
    # pos = range(len(nobs))
    # for tick, label in zip(pos, g.get_xticklabels()):
    #    g.text(pos[tick], df_mean[tick], nobs[tick],
    ...
    # plt.savefig(f'{pn}_cprdiff_enhancer_central1kbp_nice.pdf', bbox_inches='tight')
    ...
    plt.savefig(f'{p}_cprdiff_enhancer_central1kbp_nice.pdf', bbox_inches='tight')
    plt.close()