...
paths = list(set(sorted([ os.path.basename(m).split('.')[0] for m in glob.glob('../*.enhancer.5k.100b.gz')])))

for p in paths:
    mats = sorted(glob.glob(f'../{p}.d*.enhancer.5k.100b.gz'))
    ...
    for m in mats:
        ...
        df_center = pd.concat([df_combined.iloc[:, 45:55].mean(axis=1), df_combined.iloc[:,100:]], axis=1)#.groupby([120,121])#.mean()

        df_cetersub = pd.DataFrame()

        df_cetersub[f"{p}".split('_')[0]] = df_center[(df_center[100] == f'{pn}') & (df_center[101] == f'NSD2i.day_{day}')].reset_index()[0] \
                    / df_center[(df_center[100] == f'{pn}') & (df_center[101] == f'vehicle.day_{day}')][0].to_list()

        df_cetersub['Controls'] = df_center[(df_center[100] == 'Controls') & (df_center[101] == f'NSD2i.day_{day}')].reset_index()[0] \
                    / df_center[(df_center[100] == 'Controls') & (df_center[101] == f'vehicle.day_{day}')][0].to_list()        
    ...
    ## log transfrom
    g = sns.catplot(data=mat_df_long, kind="violin",palette=["#87CEFA", "#CDC9C9"],
    ...
    plt.savefig(f'{p}_ratio_enhancer_central1kbp_nice_log.pdf', bbox_inches='tight')
    plt.close()
