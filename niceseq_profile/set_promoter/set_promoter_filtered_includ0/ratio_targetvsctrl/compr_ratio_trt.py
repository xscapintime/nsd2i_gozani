...
def split_and_assign(df, row_split, col_split, row_group, col_group):
    split_df = df.loc[row_split, col_split].copy()
...
for p in paths:
    ...
    for m in mats:
        ...
        split_dfs = [
            # keep the transcript names
            split_and_assign(df.loc[:, ~df.columns.isin([4,5])], row_split, col_split, row_group, col_group)
        ...
        set_df = pd.read_csv(f'../../../../path_vsctrl_filtered_noncrna/ctrlsets_include0/{p}_ctrl_genes.txt', sep='\t')
        mapping_list =  [ dict(zip(set_df[ctrl], set_df[p])) for ctrl in set_df.columns[:-1] ]
        mapping_dict = {k: v for d in mapping_list for k, v in d.items()}

        print(f'Gene number in {p}: {len(set(mapping_dict.values()))}')
        print(f'Gene number in the control set: {len(set(mapping_dict.keys()))}')


        # merge genes in the control sets
        split_dfs[2]['gene'] = split_dfs[2][3].str.split('.').str[0]
        split_dfs[2] = split_dfs[2].groupby('gene').mean().loc[:, 6:].reset_index()
        split_dfs[2]['refgene'] = split_dfs[2]['gene'].map(mapping_dict)

        split_dfs[3]['gene'] = split_dfs[3][3].str.split('.').str[0]
        split_dfs[3] = split_dfs[3].groupby('gene').mean().loc[:, 6:].reset_index()
        split_dfs[3]['refgene'] = split_dfs[3]['gene'].map(mapping_dict)
    ...
    plt.savefig(f'{p}_cprratio_tss_central500bp_nice.pdf', bbox_inches='tight')
    plt.close()