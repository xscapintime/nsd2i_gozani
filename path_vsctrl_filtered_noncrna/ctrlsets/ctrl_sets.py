...
tpm_noncrna = tpm[~tpm.index.str.contains('ELOCP|BLOC1S2P1|BLOC1S6P1|^LINC0|^MIR|^RNU|^MT-|^MTND|^MTCO|^MTAT|-AS.$|-AS$|\.')]
...
gene_filtered = pd.DataFrame(list(set(gennehancer.index) & set(tpm.index)))
gene_filtered.columns = ['symbol']
gene_filtered['enhancer_n'] = gene_filtered['symbol'].map(enhancer_count)
gene_filtered.sort_values(by=['enhancer_n', 'symbol'], ascending=False).to_csv('gene_filtered_enhancer_count.txt', index=False, sep='\t')


# no ncRNA
gene_noncrna = pd.DataFrame(list(set(gennehancer.index) & set(tpm_noncrna.index)))
gene_noncrna.columns = ['symbol']
gene_noncrna['enhancer_n'] = gene_noncrna['symbol'].map(enhancer_count)
...
gene_uni = set(gennehancer.index) & set(tpm_noncrna.index)
