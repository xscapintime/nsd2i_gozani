...
anno_count = pd.read_csv('../../rnaseq/tpm/annotated_combined.counts', header=0, sep='\t')
ensembl_syb = dict(zip(anno_count['id'], anno_count['symbol']))
...
## filter RNA genes, ncRNA, MT genes, pseudogenes
...
# export filtered genes and enhancer count
# filter genes with low count
gene_filtered = pd.DataFrame(tpm.index)
gene_filtered.columns = ['symbol']
gene_filtered['enhancer_n'] = gene_filtered['symbol'].map(enhancer_count).fillna(0)
gene_filtered.sort_values(by=['enhancer_n', 'symbol'], ascending=False).to_csv('gene_filtered_enhancer_count.txt', index=False, sep='\t')


# no ncRNA
gene_noncrna = pd.DataFrame(tpm_noncrna.index)
gene_noncrna.columns = ['symbol']
gene_noncrna['enhancer_n'] = gene_noncrna['symbol'].map(enhancer_count).fillna(0)
gene_noncrna.sort_values(by=['enhancer_n', 'symbol'], ascending=False).to_csv('gene_noncrna_enhancer_count.txt', index=False, sep='\t')


## merged gene set
# gene_uni = tpm.index.to_list()
gene_uni = tpm_noncrna.index
...
# kras leading edge
# kras_leading = ['PLAT','CPE','GLRX','SPON1','ST6GAL1','SCN1B','SERPINA3','PSMB8','CFB','GPRC5B','CD37','PCSK1N','RGS16','ETV1','TSPAN1',
# 'NRP1','ACE','RABGAP1L','HKDC1','PLAU','VWA5A','TRIB2','FUCA1','TSPAN13','ATG10','DUSP6','IL1B','DCBLD2','ANKH','SPRY2',
# 'TRIB1','CROT','ANGPTL4','ZNF277','GFPT2','ITGA2','CAB39L','F2RL1','BTBD3','EVI5','EMP1','AMMECR1','ADAM8','LAPTM5','ALDH1A3']
