...
for m in glob.glob('../*.tss.10k.gz'):
    ...
    df_center = pd.concat([df_combined.iloc[:, 44:56].mean(axis=1), df_combined.iloc[:,100:]], axis=1)
    ...
    ## log transfrom
    g = sns.catplot(data=long_df, kind="violin",palette=["#00C5CD", "#CDC9C9"],
    ...
    g.set_ylabels('H3K4me3 (log scale)')

    g.fig.suptitle(f'{pn}'.split('.')[0], y=1.02, fontsize=12)

    plt.savefig(f'{pn}_tss_central2.4kbp_k4me3_log.pdf', bbox_inches='tight')
    plt.close()
