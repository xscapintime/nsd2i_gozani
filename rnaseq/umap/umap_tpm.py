...
for pn in ['KRAS_SIGNALING_UP', 'KRAS_SIGNALING_DN', 'EPITHELIAL_MESENCHYMAL_TRANSITION', 'INTERFERON_GAMMA_RESPONSE', 'E2F_TARGETS',
    ...
    for g in rep_merged.columns:
        ...
        ax = sns.scatterplot(x='UMAP1', y='UMAP2',
                        data=embedding[~embedding.index.isin(path)],
        ...
        ax = sns.scatterplot(x='UMAP1', y='UMAP2',
                        data=embedding[embedding.index.isin(path)],
                            hue=rep_merged.loc[rep_merged.index.isin(path),g].to_list(),
        ...
        # plt.show()
        plt.tight_layout()
        plt.savefig(f'umap_{g}_{pn}.png')
        plt.close()
